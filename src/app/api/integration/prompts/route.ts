...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${VISIBILITY_URL}/api/projects/${projectPk}/prompts`, {
      headers,
    ...
    return NextResponse.json({ error: `Failed to fetch prompts: ${(err as Error).message}` }, { status: 502 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to create prompt: ${(err as Error).message}` }, { status: 502 });
  }
}
