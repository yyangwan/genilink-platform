...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to fetch schedules: ${(err as Error).message}` }, { status: 502 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to create schedule: ${(err as Error).message}` }, { status: 502 });
  }
}
