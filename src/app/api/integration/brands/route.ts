...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${VISIBILITY_URL}/api/projects/${projectPk}/brands`, {
      headers,
    ...
    return NextResponse.json({ error: `Failed to fetch brands: ${(err as Error).message}` }, { status: 502 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to create brand: ${(err as Error).message}` }, { status: 502 });
  }
}
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to update brands: ${(err as Error).message}` }, { status: 502 });
  }
}
