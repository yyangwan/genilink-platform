...
export async function DELETE(
  ...
  try {
    ...
    const res = await fetch(`${VISIBILITY_URL}/api/projects/${projectPk}/brands/${id}`, {
      method: 'DELETE',
      headers,
    ...
    return NextResponse.json({ error: `Failed to delete brand: ${(err as Error).message}` }, { status: 502 });
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to update brand: ${(err as Error).message}` }, { status: 502 });
  }
}
