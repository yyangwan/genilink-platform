...
const INDUSTRY_OPTIONS = ["鐢靛晢", "閲戣瀺", "鍖荤枟", "鏁欒偛", "绉戞妧", "鏃呮父", "鍏朵粬"];
...
export function ProjectWizard() {
  ...
  const stepLabels = ["椤圭洰淇℃伅", "浜у搧淇℃伅", isEdit ? "纭缂栬緫" : "纭鍒涘缓"];
  ...
  return (
    ...
    >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 20px" }}>
          {step === 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <label style={labelStyle}>椤圭洰鍚嶇О *</label>
                <input
                  style={inputStyle}
                  value={form.name}
              ...
              <div>
                <label style={labelStyle}>缃戠珯 URL</label>
                <input
                  style={inputStyle}
                  value={form.url}
              ...
              <div>
                <label style={labelStyle}>琛屼笟</label>
                <select
                  style={inputStyle}
                  value={form.industry}
                  onChange={(e) => setForm((f) => ({ ...f, industry: e.target.value }))}
                >
                  <option value="">閫夋嫨琛屼笟</option>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <label style={labelStyle}>浜у搧鍚嶇О</label>
                <input
                  style={inputStyle}
              ...
              <div>
                <label style={labelStyle}>浜у搧鎻忚堪</label>
                <textarea
                  style={{ ...inputStyle, minHeight: 80, resize: "vertical" }}
                  value={form.productDescription}
                  onChange={(e) => setForm((f) => ({ ...f, productDescription: e.target.value }))}
                  placeholder="简要描述产品特性"
                />
              </div>
              <div>
                <label style={labelStyle}>浜у搧 URL</label>
                <input
                  style={inputStyle}
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {step > 0 && (
              ...
              >
                <ChevronLeft size={14} /> 涓婁竴姝?              </button>
            )}
            {step < 2 ? (
              <button
                onClick={next}
      ...
      >
        ...
        {/* Mobile step content 鈥?reuse same steps */}
        <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
          {/* Same content as desktop 鈥?simplified for mobile */}
          {step === 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <label style={labelStyle}>椤圭洰鍚嶇О *</label>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div>
                <label style={labelStyle}>浜у搧鎻忚堪</label>
                <textarea style={{ ...inputStyle, fontSize: 16, minHeight: 80, resize: "vertical" }} value={form.productDescription} onChange={(e) => setForm((f) => ({ ...f, productDescription: e.target.value }))} placeholder="简要描述产品特性" />
              </div>
              <div>
                <label style={labelStyle}>浜у搧 URL</label>
          ...
          {step === 2 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ fontSize: 13, lineHeight: 1.6 }}>
                <strong style={{ color: "var(--text-primary)" }}>椤圭洰淇℃伅</strong>
              ...
              <div style={{ borderTop: "1px solid var(--border)", paddingTop: 16, fontSize: 13, lineHeight: 1.6 }}>
                <strong style={{ color: "var(--text-primary)" }}>浜у搧淇℃伅</strong>
        ...
        <div style={{ display: "flex", gap: 8, padding: "16px", borderTop: "1px solid var(--border)" }}>
          ...
          {step < 2 ? (
            ...
            <button onClick={handleSubmit} disabled={submitting} style={{ flex: 1, padding: "12px", background: "var(--color-primary)", border: "none", borderRadius: "var(--radius-md)", color: "#fff", cursor: submitting ? "not-allowed" : "pointer", fontSize: 14, fontWeight: 600, fontFamily: "var(--font-body)", opacity: submitting ? 0.7 : 1 }}>
              {submitting ? "鎻愪氦涓?.." : isEdit ? "淇濆瓨鏇存敼" : "鍒涘缓椤圭洰"}
            </button>
          )}
        </div>
      </div>
